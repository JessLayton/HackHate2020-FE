    ...
    <FormGroup>
      <Grid container direction='row' spacing={1}>
        <Grid container item xs={12} sm={6} direction='column' spacing={1}>
          {boroughs.slice(0, Math.floor(boroughs.length / 2)).map((borough, index) => (
            <Grid item key={index}>
              <CheckBox label={borough.label} name={borough.label} checked={checkedItems[index]} onChange={() => handleChange(index)} />
            </Grid>
          ))}
        </Grid>
        <Grid container item xs={12} sm={6} direction='column' spacing={1}>
