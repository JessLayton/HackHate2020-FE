...
const Form = () => {
  ...
  const [referralsCount, setReferralsCount] = React.useState(createInitialState(referralsAndEnquiries));
  const [supportCount, setSupportCount] = React.useState(createInitialState(supportTypes));
  const [unreportedCaseCount, setUnreportedCaseCount] = React.useState(createInitialState(unreportedCases));
  const [ethnicityCount, setEthnicityCount] = React.useState(createInitialState(ethnicities));
  const [identityCount, setIdentityCount] = React.useState(createInitialState(gendersAndSexualities));
  const [caseAttributeCount, setAttributeCount] = React.useState(createInitialState(caseAttributes));
    ...
    <form className={classes.form}>
      <Grid container direction="column">
        <Grid item>
          ...
          <NumberFieldsGroup inputs={referralsAndEnquiries} value={referralsCount} onChange={setReferralsCount} minValue={0} />
        </Grid>
        <Grid item>
        <p>
        ...
        <NumberFieldsGroup inputs={supportTypes} value={supportCount} onChange={setSupportCount} minValue={0}/>
        <p>
          7. Cases not reported to Police - Please state main reasons why, with
          number of cases
        </p>
        <NumberFieldsGroup inputs={unreportedCases} value={unreportedCaseCount} onChange={setUnreportedCaseCount} minValue={0}/>
        <TextField
          label="Give details"
          variant="filled"
          InputLabelProps={{
        ...
        <NumberFieldsGroup inputs={ageGroups} value={ageCount} onChange={setAgeCount} minValue={0}/>
           <p>9. Ethnicity of persons being supported</p>
        <NumberFieldsGroup inputs={ethnicities} value={ethnicityCount} onChange={setEthnicityCount} minValue={0}/>
        <p>10. Gender / Sexual orientation of persons supported, by %</p>
        <NumberFieldsGroup inputs={gendersAndSexualities} value={identityCount} onChange={setIdentityCount} minValue={0} maxValue={100}/>
        <p>
          11. Of the cases being dealt with, how many were related to the below
          - Please provide a number
        </p>
        <NumberFieldsGroup inputs={caseAttributes} value={caseAttributeCount} onChange={setAttributeCount} minValue={0}/>
        </Grid>
        <Grid item>
        <p>
